...
const Cookie = () => {
  ...
  useEffect(() => {
    const cookieConsent = Cookies.get('zmc-cookie');
    if (!cookieConsent) {
      setIsVisible(true);
    }
  }, []);
  ...
  const handleAcceptAll = () => {
    Cookies.set('zmc-cookie', 'Alle akzeptieren', { expires: 1 });
    setIsVisible(false); // Close the popup
  };

  const handleAcceptEssentials = () => {
    Cookies.set('zmc-cookie', 'Nur essenzielle Cookies akzeptieren', { expires: 1 });
    setIsVisible(false); // Close the popup
  };
